...
public class MetaBalls {
  ...
  public MetaBalls(int size, Random random) {
    ...
    for (int i = 0; i < balls.length; i++) {
      ...
      this.balls[i] = new Vector4f((random.nextFloat() - random.nextFloat()) * size * 0.5f, (random.nextFloat() - random.nextFloat()) * size * 0.8f, (random.nextFloat() - random.nextFloat()) * size * 0.8f, ballSize);
    }
  }
