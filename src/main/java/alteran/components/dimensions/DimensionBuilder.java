...
public class DimensionBuilder {
	public static final Function<MinecraftServer, IChunkStatusListenerFactory> CHUNK_STATUS_LISTENER_FACTORY_FIELD = ReflectionUtils.getInstanceFieldGetter(MinecraftServer.class, "field_213220_d");
	...
	public static final Function<MinecraftServer, SaveFormat.LevelSave> ANVIL_CONVERTER_FOR_ANVIL_FILE_FIELD = ReflectionUtils.getInstanceFieldGetter(MinecraftServer.class, "field_71310_m");
	...
	public static ServerWorld getOrCreateWorld(MinecraftServer server, RegistryKey<World> worldKey, BiFunction<MinecraftServer, RegistryKey<Dimension>, Dimension> dimensionFactory) {
		...
		@SuppressWarnings("deprecation") Map<RegistryKey<World>, ServerWorld> map = server.forgeGetWorldMap();
	...
	private static ServerWorld createAndRegisterWorldAndDimension(MinecraftServer server, Map<RegistryKey<World>, ServerWorld> map, RegistryKey<World> worldKey, BiFunction<MinecraftServer, RegistryKey<Dimension>, Dimension> dimensionFactory) {
		...
		ServerWorld newWorld = new ServerWorld(server, executor, levelSave, derivedWorldInfo, worldKey, dimension.type(), chunkListener, dimension.generator(), dimensionGeneratorSettings.isDebug(), BiomeManager.obfuscateSeed(dimensionGeneratorSettings.seed()), ImmutableList.of(), // "special spawn list"
		...
		AlteranNetwork.get().send(PacketDistributor.ALL.noArg(), new PacketDimensionUpdate(worldKey, true));
