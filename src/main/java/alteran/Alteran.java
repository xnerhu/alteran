...
public class Alteran {
  ...
  public void onLivingTick(LivingEvent.LivingUpdateEvent e) {
    ...
    if (entity instanceof PlayerEntity) {
      if (!SpaceSystemManager.get().isEntityInSpace(entity)) return;

      PlayerDataCapability data = entity.getCapability(AlteranCapabilities.PLAYER_DATA).orElse(null);

      if (!data.hasMomentum()) return;

      Vector3d momentum = data.getMomentum();

      if (momentum.x != 0 || momentum.y != 0 || momentum.z != 0) {
        entity.setDeltaMovement(momentum.x, momentum.y, momentum.z);
      }
    }
  }
  ...
  public void onLivingUpdate(LivingEvent.LivingUpdateEvent e) {
    ...
    if (entity instanceof PlayerEntity) {
      ...
      if (entity.isOnGround() || ((PlayerEntity) entity).abilities.flying) {
        data.clearMomentum();
      } else if (!data.hasMomentum()) {
        Vector3d forward = entity.getForward();
        double speed = entity.getSpeed();

        data.setMomentum(new Vector3d(speed * forward.x, 0.25 * speed * forward.y, speed * forward.z));
      }
    }
  }
