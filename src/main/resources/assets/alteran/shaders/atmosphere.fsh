...
// scatter const
const float R_INNER = 1.5;
const float R = R_INNER + 0.2;
...
{
	// default ray dir
	vec3 dir = ray_dir( 45.0, vec2(3440.0, 1440.0), gl_FragCoord.xy );
	
	// default ray origin
	vec3 eye = vec3( 0.0, 0.0, 50.0 );

	// rotate camera
	mat3 rot = rot3xy( vec2( 0.0,900.0 * 0.5 ) );
	dir = rot * dir;
	eye = rot * eye;
	...
	gl_FragColor = vec4( pow( I, vec3( 1.0 / 1.1 ) ), 1.0 );
}